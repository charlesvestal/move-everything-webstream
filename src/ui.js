...
import { createAction, createInfo } from '/data/UserData/schwung/shared/menu_items.mjs';
...
const SAMPLETTE_GENRES = [
  'All', 'Blues', 'Brass & Military', "Children's", 'Classical',
...
function openCratedigCountryMenu() {
  ...
  for (const group of CRATEDIG_COUNTRIES) {
    if (group.region) {
      items.push(createInfo('--- ' + group.region + ' ---', ''));
    }
    for (const c of group.countries) {
      const prefix = (c === current) ? '> ' : '  ';
      items.push(createAction(prefix + c, function() {
